...
public final class FrontendChannelInboundHandler extends ChannelInboundHandlerAdapter {
    ...
    private void executeCommand(final ChannelHandlerContext context, final MySQLPacketPayload payload) {
        ...
        if (!sql.contains("@")) {
            ...
            int columnCount = selectStatement.getColumnNames().getColumnNameSegmentList().size();
            String tableName = selectStatement.getFromClause().getTableNameSegment().getIdentifier().getValue();
            int sequenceId = 0;
            sequenceId = sequenceId + 1;
            context.write(new MySQLFieldCountPacket(sequenceId, columnCount));
            List<Object> arr = new ArrayList<>();
            for (ColumnNameSegment columnNameSegment : selectStatement.getColumnNames().getColumnNameSegmentList()) {
                String columnName = columnNameSegment.getIdentifier().getValue();
                sequenceId = sequenceId + 1;
                context.write(new MySQLColumnDefinition41Packet(sequenceId, 0, "sharding_db", tableName, tableName, columnName, columnName, 100, MySQLColumnType.MYSQL_TYPE_STRING,0));
                arr.add("100");
            }
            sequenceId = sequenceId + 1;
            context.write(new MySQLEofPacket(sequenceId));
            sequenceId = sequenceId + 1;
            context.write(new MySQLTextResultSetRowPacket(sequenceId, arr));
            sequenceId = sequenceId + 1;
            context.write(new MySQLEofPacket(sequenceId));
            context.flush();
        } else {
